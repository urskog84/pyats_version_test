...
class NxosDualRpConnectionProvider(GenericDualRpConnectionProvider):
    ...
    def designate_handles(self):
        ...
        #output = con.execute('sh redundancy stat', target='a')
        con.a.spawn.sendline('show system redundancy status')
        pattern = con.a.state_machine.get_state('enable').pattern
        output = con.a.spawn.expect(pattern).match_output
        import re
